...
public class SistemaAcademico extends JFrame {
    ...
    private JPanel criarAbaDadosPessoais() {
        ...
        JButton btnLimparDados = criarBotaoImagem("limpar.png", "Limpar dados pessoais", e -> limparCamposAlunoComAviso());
        btnLimparDados.setBounds(465, 225, 58, 54);
        painel.add(btnLimparDados);
    ...
    private JPanel criarAbaNotasFaltas() {
        ...
        JButton btnLimparNotas = criarBotaoImagem("limpar.png", "Limpar campos", e -> limparCamposNotasComAviso());
        btnLimparNotas.setBounds(500, 140, 58, 54);
        painel.add(btnLimparNotas);
    ...
    private void adicionarBotoesImagemNotas(JPanel painel) {
        ...
        btnConsultar.setBounds(20, 220, 64, 60);
        painel.add(btnConsultar);

        JButton btnSalvar = criarBotaoImagem("salvar.png", "Salvar nota e faltas", e -> salvarNotaFalta());
        btnSalvar.setBounds(105, 220, 64, 60);
        painel.add(btnSalvar);

        JButton btnExcluir = criarBotaoImagem("excluir.png", "Excluir notas e faltas", e -> excluirNotasFaltas());
        btnExcluir.setBounds(190, 220, 64, 60);
        painel.add(btnExcluir);

        JButton btnJava = criarBotaoImagem("java.png", "Sobre", e -> JOptionPane.showMessageDialog(this, "Sistema Acadêmico Java + MySQL"));
        btnJava.setBounds(275, 220, 64, 60);
        painel.add(btnJava);

        JButton btnSair = criarBotaoImagem("sair.png", "Sair", e -> sair());
        btnSair.setBounds(360, 220, 64, 60);
        painel.add(btnSair);
    }

    private String[] gerarNotas() {
        String[] notas = new String[41];
        int pos = 0;

        for (int i = 0; i <= 40; i++) {
            double nota = i * 0.25;
            notas[pos++] = formatarNotaCombo(nota);
        }

        return notas;
    }

    private String formatarNotaCombo(double nota) {
        String valor = String.format(java.util.Locale.US, "%.2f", nota).replace('.', ',');

        while (valor.endsWith("0") && !valor.endsWith(",0")) {
    ...
    private void aplicarFiltroNota() {
        JTextField editorNota = (JTextField) cbNota.getEditor().getEditorComponent();
        ((AbstractDocument) editorNota.getDocument()).setDocumentFilter(new FiltroNota(this, "Nota", 4));
    }
    ...
    private void salvarNotaFalta() {
        ...
        if (Math.round(nota * 100) % 25 != 0) {
            JOptionPane.showMessageDialog(this, "A nota deve ser lançada de 0,25 em 0,25. Exemplo: 7,25 ou 8,5.");
            return;
        }
    ...
    private void gerarBoletim() {
        ...
        try {
            ...
            if (reprovacoes == 0) {
                lblBoletimStatus.setText("Status: APROVADO | Média: " + formatarNota(media) + " | Faltas: " + somaFaltas);
                lblBoletimStatus.setForeground(new Color(0, 120, 0));
            } else {
                lblBoletimStatus.setText("Status: ATENÇÃO | Média: " + formatarNota(media) + " | Reprovações: " + reprovacoes);
    ...
    private String calcularSituacao(double nota, int faltas) {
        ...
        if (nota < 6.0 && faltas >= 15) return "Reprovado";
        if (nota < 6.0) return "Reprovado";
    ...
    private boolean validarAluno() {
        ...
        String erroDataNascimento = validarDataNascimento(txtDataNasc.getText().trim());
        if (erroDataNascimento != null) {
            JOptionPane.showMessageDialog(this, erroDataNascimento);
            return false;
        }
    ...
    private String validarDataNascimento(String data) {
        try {
            ...
            if (nascimento.isAfter(hoje)) {
                return "Data de nascimento inválida. Não é permitido usar data futura.";
            }

            int idade = Period.between(nascimento, hoje).getYears();
            if (idade < 18) {
                return "O aluno deve ter no mínimo 18 anos para ser cadastrado.";
            }
    ...
    private void limparCamposAlunoComAviso() {
        limparCamposAluno();
        JOptionPane.showMessageDialog(this, "Campos dos dados pessoais limpos!");
    }
    ...
    private void limparCamposNotas() {
        ...
        cbNota.setSelectedItem("0,0");
        txtFaltas.setText("");
    }

    private void limparCamposNotasComAviso() {
        limparCamposNotas();
        JOptionPane.showMessageDialog(this, "Campos de notas e faltas limpos!");
    }
